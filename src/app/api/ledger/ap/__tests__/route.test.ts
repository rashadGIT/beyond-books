...
jest.mock('@/lib/quickbooksService', () => ({
  getQuickBooksService: jest.fn().mockReturnValue({
    getApSubledgerForConnection: jest.fn().mockResolvedValue([
      { vendorName: 'Supplier A', totalOwed: 500 },
    ]),
  }),
}));

describe('GET /api/ledger/ap', () => {
  it('returns 401 when x-user-id is missing', async () => {
    const req = new NextRequest('http://localhost/api/ledger/ap');
  ...
  it('returns 200 with AP data on success', async () => {
  ...
  it('returns 500 when QB service throws', async () => {
    ...
    getQuickBooksService.mockReturnValueOnce({
      getApSubledgerForConnection: jest.fn().mockRejectedValueOnce(new Error('QB error')),
    });
